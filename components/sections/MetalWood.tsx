      ...
      <div className="shell">
        <div className="grid-12 gap-y-10 border-t border-line pt-(--section-y)">
          <div className="col-span-4 md:col-span-8 lg:col-span-6">
            ...
            <SplitLines id="metal-heading" lines={["Metal and Wooden", "Surface Painting"]} className="mt-8 text-h2" />
          </div>
        </div>

        <div className="mt-[clamp(3rem,6vw,5rem)] grid-12 gap-y-20">
          {/* Metal */}
          <article className="col-span-4 md:col-span-8 lg:col-span-7" aria-labelledby="metal-title">
            <div className="grid gap-[var(--gutter)] md:grid-cols-[minmax(0,5fr)_minmax(0,6fr)]">
              ...
              <div>
                ...
                <ItemList items={metal.items} columns={1} />
              </div>
            </div>

            <div className="mt-12">
              <p className="eyebrow text-muted">Typical process</p>
              <Stagger as="ol" stagger={0.06} className="mt-5 flex flex-wrap items-center gap-y-3">
                {metal.process.map((step, i) => (
                  <StaggerItem key={step} className="flex items-center">
                    <span className="border border-line-strong bg-white/70 px-3 py-2 text-small">
                      <span className="mr-2 index text-muted">{String(i + 1).padStart(2, "0")}</span>
                      {step}
                    </span>
                    {i < metal.process.length - 1 ? <span aria-hidden className="mx-2 h-px w-5 bg-brass" /> : null}
                  </StaggerItem>
                ))}
              </Stagger>
            </div>
          ...
          <article className="col-span-4 md:col-span-8 lg:col-span-4 lg:col-start-9" aria-labelledby="wood-title">
            ...
            <ItemList items={wood.items} columns={1} />
          </article>
        </div>
      </div>
    </section>
